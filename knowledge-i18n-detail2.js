...
(function () {
  var DETAIL = {
    ...
    'battery-charger': {
      en: {
        principles: 'Pre-charge (trickle): below roughly 3.0V the cell is woken up with about 0.1C only, because a deeply discharged cell is damaged by full current. CC stage: past the pre-charge threshold the charger holds 0.5C-1C of constant current and the terminal voltage climbs to 4.2V. CV stage: the voltage is clamped at 4.2V, the current decays on its own, and charging stops at the programmed termination current (0.05C-0.1C). Division of protection: the charger IC covers overcharge, charge overcurrent, input overvoltage and the temperature zones read from the NTC/TS pin; over-discharge protection is not in the charger IC but in the battery protection board (PCM/BMS), and both layers are required.',
        keyFormulas: ['Pre-charge: I_pre ≈ 0.1C (V_bat below about 3.0V)', 'CC: I_cc = 0.5C-1C constant, V_bat rising', 'CV: V_bat clamped at 4.2V (needs ±1% or better), I decaying', 'Termination: I_term = 0.05C-0.1C (a smaller setting means a longer CV tail)', 'CC duration t_cc ≈ (C_bat × ΔSOC) / I_cc', 'Charge-current programming I_chg = K_iset / R_iset (K from that IC datasheet)', 'Linear charger dissipation P = (V_in − V_bat) × I_chg (thermal budget)'],
        designNotes: ['Program the charge current with the ISET/PROG resistor using the K value of that specific IC, never a formula borrowed from another part', 'Tie TS/NTC to a thermistor on the pack for JEITA temperature zones: reduce current when cold, reduce voltage or pause when hot', 'STAT is usually open-drain: it needs a pull-up, and an LED needs its own series resistor. Never leave it floating', 'A linear charger burns the whole headroom as heat: move to a switching (buck) charger when (V_in − V_bat) × I_chg gets large', 'The input needs overvoltage protection and an input capacitor; the BAT-side capacitor follows the datasheet. Skip neither', 'The protection board (over-discharge, secondary overcharge, short circuit) and the charger IC cover different failures, so keep both'],
        commonMistakes: ['Charging from a fixed 4.2V supply or an LDO: no CC current limit and no termination, so the cell is overcharged', 'Treating over-discharge protection as a charger-IC feature when it belongs to the protection board', 'Driving an LED from STAT with no series resistor, or forgetting the pull-up', 'Charging below 0°C: lithium plating permanently degrades the cell (violates the JEITA zones)', 'CV accuracy worse than ±1%: a permanently high float overcharges and cuts cycle life', 'Too little copper for a linear charger: thermal foldback stops it topping off and the battery gets blamed']
      },
      ja: {
        principles: 'プリチャージ（トリクル）：電池電圧が約 3.0V 以下のときは約 0.1C の小電流だけでセルを起こす。深放電したセルにいきなり大電流を流すと劣化する。CC（定電流）：しきい値を超えたら 0.5C～1C の定電流で充電し、端子電圧は 4.2V まで上昇する。CV（定電圧）：電圧を 4.2V にクランプすると電流は自然に減衰し、設定した充電終止電流（0.05C～0.1C）で停止する。保護の分担：充電 IC が担うのは過充電・充電過電流・入力過電圧と NTC/TS ピンによる温度ゾーン。過放電保護は充電 IC ではなく電池保護基板（PCM/BMS）の役割で、両方が必要。',
        keyFormulas: ['プリチャージ：I_pre ≈ 0.1C（V_bat が約 3.0V 以下）', 'CC：I_cc = 0.5C～1C 一定、V_bat は上昇', 'CV：V_bat を 4.2V にクランプ（精度 ±1% 以内が必要）、I は減衰', '終止：I_term = 0.05C～0.1C（小さく設定するほど CV 尾部が長い）', 'CC 時間 t_cc ≈ (C_bat × ΔSOC) / I_cc', '充電電流設定 I_chg = K_iset / R_iset（K は該当 IC のデータシート）', 'リニア充電 IC の損失 P = (V_in − V_bat) × I_chg（放熱設計の根拠）'],
        designNotes: ['充電電流は ISET/PROG 抵抗で設定。K 値はその IC のデータシートに従い、他品種の式を流用しない', 'TS/NTC はパックのサーミスタへ接続し JEITA 温度ゾーンを実装：低温では電流を下げ、高温では電圧を下げるか一時停止', 'STAT は多くがオープンドレイン：プルアップが必要、LED には別途直列抵抗。未接続のまま使わない', 'リニア充電 IC は電圧差がすべて熱：(V_in − V_bat) × I_chg が大きいならスイッチング（降圧）充電 IC に変更', '入力側に過電圧保護と入力コンデンサ、BAT 側コンデンサはデータシート通りに。どちらも省かない', '電池保護基板（過放電・二次過充電・短絡）と充電 IC は守備範囲が違うので両方実装する'],
        commonMistakes: ['固定 4.2V 電源や LDO で直接充電：CC 段の電流制限も終止判定もなく過充電になる', '過放電保護を充電 IC の機能と勘違いする。それは保護基板の役割', 'STAT に直接 LED を付けて限流抵抗なし、またはプルアップ忘れ', '0°C 未満でも通常充電：リチウム析出でセルが恒久劣化（JEITA ゾーン違反）', 'CV 精度が ±1% より悪い：常時高めで過充電となりサイクル寿命が低下', 'リニア充電 IC の放熱銅箔不足：サーマルフォールドバックで満充電できず電池不良と誤診']
      },
      ko: {
        principles: '예비 충전(트리클): 전지 전압이 약 3.0V 미만이면 약 0.1C의 작은 전류로만 셀을 깨운다. 과방전된 셀에 큰 전류를 바로 넣으면 손상된다. CC(정전류): 예비 충전 문턱을 넘으면 0.5C~1C 정전류로 충전하며 단자 전압이 4.2V까지 올라간다. CV(정전압): 전압을 4.2V로 고정하면 전류가 자연히 감소하고, 설정한 종료 전류(0.05C~0.1C)에서 충전을 멈춘다. 보호 분담: 충전 IC는 과충전·충전 과전류·입력 과전압과 NTC/TS 핀 온도 구간을 담당한다. 과방전 보호는 충전 IC가 아니라 배터리 보호 회로(PCM/BMS)의 역할이며 두 계층 모두 필요하다.',

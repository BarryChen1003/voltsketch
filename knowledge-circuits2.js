...
(function () {
  ...
  M['epd-power-rails'] = () => {
    ...
    // 匯出分支：垂直支線用 LR——y 與匯流排 y=100 相距 <12px 的那條會被吸成同一點而消失
    ys.forEach((y, i) => { g += L(245, 100, 262, 100) + (y !== 100 ? LR(262, y, 262, 100) : '') + A(262, y, 285, y) + T(290, y + 3, names[i], { size: 8, anchor: 'start' }); });
  ...
  M['epd-partial-refresh'] = () => {
    ...
    g += T(344, 66, '殘影', { size: 8.5, fill: ORG });   // 距箭頭末端 8px 在 ×1.3 後只差 0.6px → 拉到 14px
  ...
  M['mobile-pmic'] = () => {
    ...
    outs.forEach(([y, lbl]) => { g += L(238, 96, 252, 96) + (y !== 96 ? LR(252, y, 252, 96) : '') + A(252, y, 272, y) + T(277, y + 3, lbl, { size: 8, anchor: 'start' }); });
  ...
  M['haptics-driver'] = () => {
    ...
    // 兩個波形標籤原本 96/130 置中會互壓（85..107 vs 91..169）；紅字說明又壓在波形右端
    g += T(80, 176, '驅動', { size: 7.5, fill: MUT }) + T(152, 176, '反向煞車→快停', { size: 7.5, fill: ORG });
    g += T(200, 198, '偏離 f0 → 效率驟降、手感差；GPIO 直推 = 無追蹤無煞車', { size: 8, fill: RED });
    return { d: 'LRA 驅動：共振頻率追蹤＋主動煞車', svg: W(400, 210, g) };
  };
  ...
  M['vrm-multiphase'] = () => {
    ...
    [40, 90, 140].forEach((y, i) => {
      ...
      // 電感原本畫在 212（跨 188..236）→ 左端戳進 DrMOS 方塊（右緣 192）。移到 220。
      g += ind(220, y, { horizontal: true, label: '' });
      g += L(244, y, 268, y);
    });
    g += T(162, 170, '相位交錯 360°/N → 漣波抵消', { size: 8, fill: MUT });
    g += L(268, 40, 268, 140) + A(268, 90, 296, 90);
    // 輸出電容陣列（頂腳貼輸出線 y=90，接地對齊底腳）
    // 原本 306/318 只差 12px，電容極板（±11）與接地橫槓（±12）整片重疊 → 拉到 30px 間距
    [296, 322].forEach(x => { g += cap(x, 112, { horizontal: false }) + gnd(x, 148, {}) + s.junction(x, 90); });
    g += T(315, 66, 'MLCC 陣列＋bulk', { size: 7.5, fill: MUT });
  ...
  M['acdc-flyback'] = () => {
    ...
    // 走線用 LR 走直角（L 會被吸附成斜線＋零長線段）。
    // MOSFET 原本畫在 (138,128)：框(108..164 × 108..148) 與 bulk 電容的接地符號
    // (120..144 × 100..124) 整塊重疊。整排往下移 22px，讓開接地符號。
    g += LR(160, 92, 160, 130) + LR(160, 130, 164, 130);
    g += mos(138, 150, { showPins: false, flip: true });
    // gnd 自帶 stem（y-14→y）已覆蓋 Source 引線末端 (164,170)，不再補線（補了只會變零長線段）
    g += gnd(164, 178, {}) + T(146, 182, 'PGND', { size: 7, fill: MUT, anchor: 'end' });
    g += B(80, 150, 56, 34, 'PWM', ['控制器']);   // 右緣 x=108＝閘極引線起點，直接相接
    // 二次側：繞組底端→接地（LR：gnd 腳位登錄在 y-14，走 L 兩端會被吸成同一點）
    g += LR(188, 92, 188, 100) + gnd(188, 114, {}) + T(206, 120, 'SGND', { size: 7, fill: MUT, anchor: 'start' });
    g += L(188, 56, 214, 56);
    ...
    // 回授路徑改走底部繞行：原本從光耦直線斜拉到 PWM，會橫穿 MOSFET 符號。
    g += LR(272, 171, 272, 194) + LR(272, 194, 80, 194) + A(80, 194, 80, 169, null, { color: ACC, w: 1 });
    // 說明文字帶：必須整條低於回授繞行線(y=194)，畫布同步加高（鐵律：圖與字不得重疊）
    g += T(196, 212, '開關閉合：能量存一次側；斷開：極性反轉、二極體導通放能到輸出', { size: 8.5 });
    g += T(196, 228, '一二次側「地」隔離（安規）；回授過光耦', { size: 8.5, fill: ORG });
    return { d: '返馳式：變壓器儲能＋光耦隔離回授', svg: W(400, 238, g) };
  };
  ...
  M['usbc-pd-sink-path'] = () => {
    ...
    g += L(72, 48, 112, 48) + T(88, 28, 'VBUS 5~20V', { size: 7.5, fill: MUT });
    // 背靠背 NFET（方塊表示，含方向註記）
  ...
  M['c2000-digital-power-control'] = () => {
    ...
    // 取樣標籤原本擺在箭頭上方 26px，直接壓在功率級方塊底部（中心落在框內，官方 checker 會當成框標題跳過）
    // → 兩條回授對調高度：CMPSS 走 y=100，取樣走 y=118，標籤夾在兩線之間
    g += A(226, 76, 226, 100) + L(226, 100, 135, 100);
    g += L(300, 56, 300, 118) + A(300, 118, 135, 118, null, { color: ACC });
    g += T(217, 112, '電壓/電流取樣（ADC 與 PWM 同步）', { size: 8, fill: ACC });
  ...
  M['clock-tree-fanout'] = () => {
    ...
    [[40, 'FPGA'], [80, 'ADC'], [120, 'SerDes']].forEach(([y, lbl]) => {
      g += L(196, 80, 212, 80) + (y !== 80 ? LR(212, y, 212, 80) : '') + L(212, y, 236, y);
  ...
  M['ethernet-phy-layout'] = () => {
    ...
    // 標籤（約 40px）比兩框之間的 38px 空隙寬 → 置中會擦到 MAC/PHY 框邊（瀏覽器實測是「壓方塊」，
    // node 估算器因字寬低估看不到）。下移到框列下方的空白帶（y 96–112）。
    g += A(120, 82, 84, 82, 'MDIO/MDC', { color: MUT, dy: 28 });

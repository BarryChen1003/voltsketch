...
node dxf.test.js          # DXF 板框：單位/弧線/封閉性/來回一趟
node crossprobe.test.js   # 線路圖↔PCB 選取連動：回音防護、訊息驗證
node step.test.js         # STEP 3D：參照完整性/流形性/尤拉示性數
node silkimg.test.js      # 圖片轉絲印：換算/合併/太細偵測
node pour.test.js         # 鋪銅孤島：連通性/挖除範圍/誤判
node blindvia.test.js     # 盲埋孔：跨層範圍/分檔/板廠能力
...
| 功能 | 實際狀況 |
|---|---|
| **PCB 進階功能** | 🟢 2026-08-24 全數釘住：等長蛇形、疊層、背鑽、autoRoute（含多層）、DRC、差分間距、走線稽核、EMI 迴路面積、熱簡估、Gerber 讀回幾何，合計 **294 + 195 + 43 + 118 + 61 + 70 + 63 + 27 + 62 + 47 + 45** 條斷言（邏輯／Gerber 讀回／鋪銅幾何／製造功能／製造匯出／線路圖轉 PCB／DXF 板框／選取連動／STEP 3D／圖片絲印／鋪銅孤島） |
...
| **STEP 3D 匯出** | 🟡 幾何與拓樸程式驗過（流形、尤拉、參照完整），但**沒有人用真的 CAD 開過**；元件是佔位方塊不是原廠模型 |
...
| STEP 匯出 | ✅ `pcb-step.js`（AP214）| 板框多邊形沿 Z 擠出板厚、元件依封裝類別擠出佔位高度。**驗得了的都驗**：參照完整性、流形性（每條邊剛好被兩個面用到且方向相反）、尤拉示性數 V−E+F=2、座標與板厚。結構檢查沒過就**不匯出**，不給使用者壞檔。⚠ 誠實界定：沒有元件外形庫，方塊只代表佔位空間；**沒有人用真的 CAD 開過**（同 KiCad 匯出的既有註記）|
| DXF 板框匯入／匯出 | ✅ `pcb-dxf.js` | **匯入後先問三件事**：① 單位（$INSUNITS 沒填就明講「假設 mm」，尺寸差 25.4 倍一眼看得出）② **板框封閉嗎**（沒封閉板廠切不出來，直接指出開口座標；EasyEDA 讓你送件被退才知道）③ 弧線弦高誤差可設定。支援 LINE/ARC/CIRCLE/LWPOLYLINE(含 bulge)/POLYLINE |
| ODB++ 匯出 | ⬜ 未做 | — |
| 線路圖 ↔ PCB 雙向 cross-probe | ✅ `pcb-crossprobe.js` | 走 BroadcastChannel（跨分頁），localStorage 為後備。對應靠 `sch-<id>` 前綴；**對不到的元件（公版／KiCad／手放）直接略過不猜**。兩個重點：回音防護（套用遠端選取時不再廣播，否則兩分頁無限彈）與訊息驗證（版本/形狀/來源不符一律丟掉）|
| 圖片轉絲印 | ✅ `pcb-silkimg.js` | 灰階→二值化→逐列區段→垂直合併成矩形→填充區域（不做輪廓向量化：矩形對 logo 夠用且精確，也不必爭論簡化參數）。**多做的**：太細的絲印印出來會斷，拿選定板廠的 `minSilkWidth` 逐塊判定，回報最細處與塊數，可選擇自動剔除。透明像素當白色（去背區不會被當成墨）|
...
## 教訓
- 2026-08-25 | 測資要能分辨實作 | 盲埋孔的跨層檢查改了四次測資才測得出對錯：路徑從第 0 層起 → 上下界等價；穿孔 pad → 根本不換層；禁佈區沒蓋到板邊 → 有合法縫可鑽 | 每寫一條斷言就問「錯的實作會讓這條紅嗎」；答不出來就是還沒測到
- 2026-08-25 | autoRoute 成功率 | 以為 70% 是壅塞造成的，加了 rip-up 才發現 23/26 的失敗是「起點就被封住」——公版多數 pad 沒有 net，密腳封裝的目標腳被自己人包死 | 先量失敗**原因**再決定要做什麼；不然會對著錯的假設寫功能
- 2026-08-25 | rip-up 幫倒忙 | 第一版寫成「拆了就繼續」，拆線連鎖把已繞好的一起帶走，20 顆元件的板從 95% 掉到 10% | 任何「先破壞再嘗試」的最佳化都要有回滾守衛，保證結果只可能變好；而且要用**會走進破壞路徑的測資**測它，單一情境測不到
- 2026-08-25 | 效能最佳化 | 先改了 PadDrc（789→714ms 幾乎沒動），量了才發現真正的瓶頸是 ConstraintMgr.audit 535ms 與 Ratsnest 的貪婪接法 | **先量再改**；而且加速改動一律要能證明「輸出逐行不變」，否則是換了行為不是加速
- 2026-08-25 | 決定性輸出 | 換成空間網格後迭代順序變了，DRC 每類上限 30 筆導致「列出哪 30 筆」跟著變 | 有數量上限的輸出，候選一律照原始索引排序，否則同一片板兩次跑出不同結果
- 2026-08-25 | 鋪銅孤島 | 跟自己網路斷開的銅，畫面上跟正常鋪銅一樣、DRC 也不報（它沒違反任何間距規則），但那是浮空天線 | 「DRC 全綠」不等於「板子對」；規則檢查只涵蓋規則寫得出來的部分，連通性這類性質要另外驗
- 2026-08-25 | mutation 測資（第二次踩） | 圖片絲印的兩個 mutation 沒被抓到：y 軸翻轉用了上下對稱的測圖、短邊/長邊用了接近正方形的測圖，兩種寫法算出同樣的值 | 每寫一個 mutation 就問「這份測資分得出兩種寫法嗎」；同型錯誤本 session 已出現兩次（前一次是鑽孔表的分岔釘子）
- 2026-08-25 | 驗不了的東西怎麼驗 | STEP 開不了 CAD，但流形性（每條邊剛好被兩個面用到且方向相反）、尤拉示性數、參照完整性都是程式驗得了的硬性質 | 「無法端到端驗證」不等於「不能驗」；先把可驗的不變式列出來，剩下的照實標成未驗證
- 2026-08-25 | script 載入順序 | `pcb-crossprobe.js` 排在 `pcb.js` 後面，`pcbApp.init()` 跑的時候 `window.CrossProbe` 還不存在，功能靜靜不啟用（沒有錯誤訊息） | 被 `init()` 直接用到的模組要排在 `pcb.js` **之前**；只在按鈕事件裡用的才可以排後面
- 2026-08-25 | 容差比對 | 端點封閉性用 `Math.round(x/tol)` 分桶，0 與 0.005 在容差 0.01 下落到不同桶（round(0.5) 進位），明明接得上卻報成兩個開口 | 「在容差內算同一點」要按**實際距離分群**，不要用格點取整代替

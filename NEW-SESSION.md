...
| 功能 | 實際狀況 |
|---|---|
| **2nd source 比對** | 🟡 **可以用了，但只在「講差異」這件事上**。2026-08-20 兩件事：(1) 抽取層重寫——版面感知取列（依 y 併列）＋單位正規化＋四道閘（等號後是測試條件、Absolute Maximum／Limiting values 整段跳過、數值必須與單位相鄰、量級不合理就不抽）；(2) 新增**換料注意事項**：把差異翻成「線路上要注意什麼」。真實案例 PCA9555A(NXP) vs PCA9535(TI)：pin-to-pin、暫存器相同，但 PCA9535 的 datasheet 自己寫著「except for the removal of the internal I/O pullup resistor」——報告現在會標紅講「每一支要外加上拉」，並抓出電源下限 1.65V→2.3V。迴歸素材 `tools/ds-compare/fixtures/`（5 份真 datasheet），測在 `ds-compare.test.js`（121 passed）。**沒有比對**：暫存器位址與上電預設、I2C 時序、驅動能力、ESD、散熱墊——報告最後一條會自己講出來。196 顆裡 49 顆仍沒有 `secondSource` 準則，但注意事項不依賴準則，缺準則也有東西看。 |
...
| 優先 | 項目 |
|---|---|
| **1** | **把「換料注意事項」再長出幾條**：目前只認得內建上拉、5V 耐受、開汲極中斷、電源／溫度範圍、腳數。還缺的高價值項目：未使用腳能不能浮接、內建 vs 外接（bootstrap 二極體、補償、軟啟動、振盪器）、熱墊是否必須接地、上電預設狀態。加法照 `iopull` 那條的樣子寫，四語字串在 `L.*.note` |

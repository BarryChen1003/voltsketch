...
| 功能 | 實際狀況 |
|---|---|
| **2nd source 比對** | ❌ **不可信，不要拿去推廣**。2026-08-20 使用者實測一組真 PDF：14 個參數只有 3 個兩邊都抽到，其餘「未擷取」；而且「靜態電流 IQ = 160 mA」明顯是誤抽（量級不可能）。另外那顆 IC 在 `ic-data.js` 沒有 `secondSource` 準則，所以「準則 0 條」＝整個判定區塊是空的。**要修的是抽取率與誤抽，不是介面。** |
...
| 優先 | 項目 |
|---|---|
| **1** | **修 2nd source 比對的抽取率與誤抽**（推廣期間這是最傷的一個） |

...
| `pcb-shove.js` | `Shove` | 推擠：把擋路的平行鄰居側推（只平移、不重繞；`planChain` 支援連鎖） |
...
| `shove.test.js` | 62 | 推擠：**不該推的時候不可推**；連鎖推擠（A 推 B、B 再推 C）與其三個守衛 |
| **`pcb-drag.test.js`** | 40 | 拖整段走線：**pad 那一端不可以被拖走**（補線防無聲斷線）、對齊輔助線 |
...
| # | 項目 | 狀態 |
|---|---|---|
| 10 | 連續多段繪製 | ✅ 收完一段從終點接著畫；收在 pad/via 或按 Esc 才結束。收筆抽成 `finishTraceSegment`，兩個入口共用同一份（不然推擠與規則檢查會有一條路漏掉） |
| 11 | 整條走線拖曳 | ✅ `pcb-drag.js`。鄰居跟著拉長；**壓在 pad/via 那一端補一小段**，不把線從 pad 上扯下來 |
| 12 | 拖曳時的對齊輔助線 | ✅ 每個軸只留最近的一條，而且真的吸過去 |
| 13 | 連鎖推擠 | ✅ `Shove.planChain`。中途不驗、整條鏈算完再驗一次（中途驗會把每個中間狀態都判失敗）。仍然只平移不重繞 |

### 批次 6 — 模擬 ✅

| # | 項目 | 狀態 |
|---|---|---|
| 14 | SPICE 探針與游標量測 | ✅ `spice-measure.js`。點波形放 A/B 游標看 Δ 與 1/Δt；自動量測表（Vpp/RMS/上升時間/頻率、AC 的 −3dB）。**量不出來顯示「—」不顯示 0** |
| 15 | 蒙地卡羅 ／ 參數掃描 | ✅ `spice-sweep.js`。種子亂數＝**同種子同結果**；公差＝3σ 並截斷；回分位數與良率，**刻意不給信心區間** |

**為什麼不給信心區間**：蒙地卡羅的樣本是同一個模型的重複求解，不是獨立實驗。
把它講成「95% 信心」是拿模擬的精度冒充現實的把握——而元件模型本身還是一階的（見 §8）。
...
| 缺什麼 | 影響 |
|---|---|
| 🟡 **推擠仍只平移、不重繞** | 連鎖可以了（2026-08-28），但擋路的線只會整條側移，不會改走法繞過去 |
...
| 🟡 **量測精度受取樣間隔限制** | 游標與自動量測都是取樣點之間的線性內插；上升時間之類的數字，精度上限就是取樣間隔（結果會附取樣數與間隔） |
| 🟡 **掃描的指標只有 AC 的 −3dB 轉角** | 那是這個工具算得最準也最常被公差影響的一個數字；換別的指標要改程式，刻意不做成使用者輸入的表達式 |
...
- 2026-08-28 | 推擠 | 驗證時把新畫的那一段又複製一份加進副本，但呼叫端已經先把它放進 `state.traces` 了——它跟自己的分身距離 0，而沒有 net 的線連 sameNet 都不成立，於是推擠**每次都判失敗**（既有 bug，測試一直沒抓到是因為測資都把 seg 放在清單外） | 測資要照**真正的呼叫方式**擺資料；「函式自己測得過」跟「呼叫端用起來對」是兩件事
- 2026-08-28 | 連鎖推擠 | 連鎖第二輪起「被推開的線去推別人」，此時使用者剛畫的那一段只是清單裡的普通線，會被推走 | 有「這個東西不准動」的概念就要明確傳下去（protect 清單），不要靠 `t === seg` 這種只在第一輪成立的判斷
- 2026-08-28 | 兩個守衛互相掩護 | 連鎖的「深度不夠」與「最後驗證」兩個守衛，拿掉任一個測試都還是綠的——因為每個案例都同時觸發兩者 | mutation 測到「拿掉還是綠」時，要補的是**只觸發其中一個**的案例；造不出來就直接測那個機制本身
- 2026-08-28 | 呼叫別人的 API | `Spice.ac` 的選項是 `start/stop/points/sweep`，我寫成 `from/to/ppd`——**沒有報錯**，全部退回預設值。node 測試因為預設範圍剛好夠寬而通過，瀏覽器上才發現 | 測試要驗「我傳的參數真的生效」（例：起始頻率就是我指定的那個），不是只驗「有結果」
- 2026-08-28 | 繞過包裝函式 | 直接呼叫 `Spice.ac` 而不是 `SchSpice.ac`，漏掉「電壓源要帶 ac 振幅」那一步 → 整條線都是 0、圖是平的、−3dB 一律量不到，而且**不會有任何錯誤訊息** | 包裝函式做的準備工作要嘛沿用它、要嘛抄過來並在註解寫明為什麼；另外加一條「探測那一輪要真的有訊號」的守衛
- 2026-08-28 | 瀏覽器測試 | 抓著重畫前的 DOM 參照做後續操作（`msg()` 會換掉 innerHTML），結果點擊送到孤兒節點上，看起來像功能壞了 | 每次互動都重新 `getElementById`，不要跨重畫重用節點參照

...
| 優先 | 項目 |
|---|---|
| **1** | **建庫流程接上驗證器**（`tools/ic-extract/`，2026-08-21 已做好）：把 datasheet 抽出來的值
產成「宣稱」（值＋出處原文＋頁碼），四道覆核過了才准進 `ic-data.js`。引擎可換（現在是規則，之後可以是模型或人工），
驗證器不換。下一步：`package`／`pins`／`interface`／`bits`／`esd`／`integ`／`aecq` 這幾條規則還沒記錄出處，補上就能全參數覆核 |
| **2** | **注意事項再長幾條**（已有 7 類，加法照 `iopull` 那條寫、四語字串在 `L.*.note`）。還缺：I2C 位址範圍是否相同、輸出驅動能力 IOL/IOH、上電時序與 POR 門檻、暫存器預設值。另有一個已知限制：兩欄排版的 datasheet 會把右欄併進同一列，跨欄的敘述抓不到（RT6150 的「internal soft-start」就是被右欄切斷，改用規格表的 tSS／fOSC 當證據繞過）。要根治得做欄位重排 |
| **3** | **補 49 顆 IC 的 `secondSource` 準則**（`ic-data.js`；用 `ic-crit-i18n-data.js` 既有片語就不必另外翻四語。準則裡要有「內建上拉一致」這類會改到外部電路的條件） |
| 4 | 腳位抽取正確率（77/193）與跨頁大表 |
| 5 | C-1 付費知識卡月更（每月 5 張）——**付費頁面的承諾，收錢後是契約義務** |
| 6 | 面試題庫擴充｜知識卡產品卡填充 |
| 7 | `page_views` 保留期清理（推廣後這張表長最快） |
...
- 2026-08-21 | 建庫要不要接模型 | 使用者問地端 LLM 會不會更好。結論：前台（訪客即時比對）不接——地端 API 訪客連不到、改雲端會打破「檔案不上傳」承諾；後台建庫值得，但關鍵不是引擎而是**驗證器**（quote 逐字存在＋數值重新解析＋量級＋不可來自絕對最大額定） | 引入任何生成式引擎之前，先把「機械覆核」寫出來；沒有覆核就沒有資格談換引擎

...
| 功能 | 實際狀況 |
|---|---|
| **2nd source 比對** | 🟡 **抽取層已重寫（2026-08-20），準則覆蓋率還沒補**。抽取改成版面感知（依 y 併列，一列＝表格的一列）＋單位正規化＋三道閘（等號後的測試條件不算值、Absolute Maximum 整段跳過、量級不合理就不抽）。三份真 datasheet 的迴歸素材放 `tools/ds-compare/fixtures/`，測在 `ds-compare.test.js`（101 passed）。實測擷取數：RT6150 9/14、W25Q128JV 7/14、AXP209 9/14，逐條對過原文，沒有已知錯值（IQ 160 mA、VCC 抓到 abs max 那類已擋掉）。**還沒好的是準則：196 顆裡 49 顆沒有 `secondSource`**（含使用者實測的 RT6150／AXP209／W25Q128JV），那幾顆只有參數表、判定區塊空的。抽不到的欄位一律留白，報告會直接寫出「這份 PDF 沒文字／料號對不上」。 |
...
| 優先 | 項目 |
|---|---|
| **1** | **補 49 顆 IC 的 `secondSource` 準則**（`ic-data.js`；抽取層已修好，缺準則的那幾顆判定區塊還是空的。用 `ic-crit-i18n-data.js` 既有片語就不必另外翻四語） |
| 2 | 腳位抽取正確率（77/193）與跨頁大表 |
| 3 | C-1 付費知識卡月更（每月 5 張）——**付費頁面的承諾，收錢後是契約義務** |
| 4 | 面試題庫擴充｜知識卡產品卡填充 |
| 5 | `page_views` 保留期清理（推廣後這張表長最快） |

...
| 功能 | 實際狀況 |
|---|---|
| **8 片公版的佈局是重建出來的** | DRC error 0（預算鎖在 `pcb-logic.test.js` 第 21 節，**全 0 只准往下**）。2026-09-01 現況：**未繞 52、零長度 6**（當日起點是未繞 134、零長度 70+）。零長度飛線＝同一點、同 net、不同層、缺 via 的壞接點：畫面上看不見，使用者只覺得飛線降不下去。根因（繞線器收線收在沒有銅的層）已修；剩下的密腳區靠**逃逸 via**解掉（via 往外挪、兩層各補短線，`AutoRoute.escapeVia`）。還剩的 4 條在連鑽孔都塞不下的地方。未繞與零長度各有預算（第 21b 節，只准往下）|

...
| 4 | net 升級成一級物件 | 🟡 已完成一半：公版帶 `padNets`、線路圖同步改成 ECO 增量、網名穩定化。還缺 net 清單 UI、未接線面板、back-annotation |
...
| 缺什麼 | 影響 | 現況 |
|---|---|---|
| 🔴 **back-annotation（PCB → 線路圖）** | 在 PCB 換封裝、改 net、swap 腳位，線路圖不知道。只能單向 | 完全沒有 |
| 🟡 **封裝在線路圖階段就綁定** | 我們在轉換時才挑預設變體（電阻預設 0603），使用者可能拿到不是他要的料 | `fpOverrides` 可事後覆寫，但線路圖裡看不到封裝 |

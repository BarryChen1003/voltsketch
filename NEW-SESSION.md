...
| 功能 | 實際狀況 |
|---|---|
| **PCB 進階功能** | 🟡 2026-08-24 已釘住：等長蛇形、疊層、背鑽、autoRoute 淨空、DRC、差分間距、走線稽核都有斷言了（242 條）。**EMI／熱檢查仍無測試**，見 §8 |
| **8 片公版自己過不了 DRC** | 🔴 **2026-08-24 實測確認，不是誤判**。合計 388 個 error：VBUS 走線直接橫越 J1 整排 pad（回報距離 `d=0`，實際就是短路）；U1 與 C3 的 pad 只隔 0.095mm，連 JLCPCB 的 0.10mm 下限都不到。公版的走線是「示意直線」不是真的繞線，幾何上確實不可製造。已用缺陷預算鎖住（`pcb-logic.test.js` 第 21 節），**只准往下不准往上**。修法＝重畫 8 片板，尚未做 |
...
`pcb-logic.test.js` 從 9 個邏輯點長到 **242 條斷言**，第 9～21 節是這一輪加的。

...
| `sch-hier.test.js` | 58 | 階層式圖紙：**多實例必須彼此隔離**、遞迴當場擋下、頂層判定 |
...
| 測試 | 斷言 | 守什麼 |
|---|---|---|
| `ipc2581.test.js` | 66 | IPC-2581 ＋ 組裝圖；第一條就是 **well-formed XML** |
...
| **推擠只平移不重繞** | 連鎖可以了（2026-08-28），但擋路的線只會整條側移，不會改走法繞過去 |
| **階層沒有交叉探測** | 2026-08-31：**雙擊圖紙符號可進入子圖**，分頁列右側有麵包屑與「⬆ 上一層」；手動點分頁列會清掉路徑（不清的話「上一層」會跳去不相干的頁）。**階層路徑其實一直都有帶進 refdes**（`SchHier.build` 產出 `PWR1/R1`，`Sch2Pcb` 直接用它當 ref）——舊版這份文件寫錯了。**還缺的**：從 PCB 反查是哪一層（cross-probe 只到 net）、子圖內選取範圍高亮 |
...
## 10. 路線圖：七批全部完成（2026-08-27 ～ 08-28）

2026-08-27 使用者核定「剩下的 15 項全部要做」。到 2026-08-28 全部完成並上線。
...
**2026-08-31 這一輪做完的**：net 點哪亮哪、右鍵選單、走線屬性面板、快捷鍵說明面板、
弧線高亮修正、hover 資訊卡、走線中換層自動打 via、CAM 螢光配色（含對比度守門）、
3D 阻焊／絲印貼圖與元件外型反推、3D 綁 `.wrl`/`.obj` 真模型、DRC 違規畫面標紅、
Touchstone (.sNp) 匯入、CSP `frame-src` 修正（板面檢視在線上是破圖）、面板選單捲軸。
...
1. **gate swap 的 UI**——`SchSwap.canSwapGates` 判準早就有了，沒有入口等於沒有這個功能。
2. **匯流排帶進 PCB 端**：轉 PCB 之後就散成一條條 net，板子不知道「這 8 條是一束」，
   等長調諧與差分/群組佈線都少了依據。
3. **net 屬性進匯出檔**：阻抗目標與成對關係目前只活在編輯器與 DRC，板廠拿到的網表沒有。
4. **階層導覽**：子圖當符號可以了，但沒有「進入子圖」，也沒把階層路徑帶進 PCB 的 refdes。
5. **組裝圖元件外形**：現在是 courtyard 方框，可以改用 `pcb-3d-shapes.js` 的封裝分類畫出真外形。
6. **推擠改成會繞路**：目前擋路的線只會整條側移，不會改走法。
7. **練習模式／教學導覽**：`pcb-practice.js`＋`pcb-tutorial.js` 13KB 死碼，沒有任何頁面載入。

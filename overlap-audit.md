...
| 批次 | 圖 | 字級 @ viewBox | 重疊 |
|---|---|---|---|
| 批次 1 新繪（q7 q12 q17 q19 q26 q27） | 6 | 11px @520 | **0** |
| 重畫（q5 q6 q13） | 3 | 11px @520 | **0** |
| flyback（q28–q32，共用底圖） | 5 | 10px @720 | **0** |
| 批次 3 新繪（q1/q4 同圖、q2 q14 q15 q16 q22） | 7 題 / 6 張 | 11px @520 | **0** |
| **合計（zh）** | **21 題** | — | **0** |

zh + en 兩份一起掃：37 張、603 個文字、重疊 0。

flyback 那五張只調字級（7→10）與六個標籤位置（Q1 / Cps / Cout / 400V / VDD / D3），
**元件座標一字未動**——`line`/`rect`/`polygon`/`path` 逐字比對與改前相同。拓樸本來就是對的，
別為了「重畫得漂亮」去動它，那是回歸風險最高的動作。

踩到的坑：Q1 第一次往左挪到 `x=436 anchor=end` 反而壓到 MOSFET 閘極引線（x=424）與
汲極引線（x=432）。最後放在汲極上方 `(436,166) anchor=start`、字級 14→12。
**挪標籤要看鄰居的實際座標，不能只看「往空的方向移」。**

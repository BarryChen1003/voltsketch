...
(function () {
  ...
  window.IC_UI = {
    zh: {
      ...
      cmpTitle: '2nd Source 比對（上傳兩份 datasheet）', cmpIntro: '上傳「現用料」與「候選料」的 datasheet PDF，產出差異報告：參數差異表 + 上方替換準則逐條判定。', cmpFileA: '現用料 PDF', cmpFileB: '候選料 PDF', cmpRun: '比對並產出報告', cmpNote: '檔案只在你的瀏覽器內解析，不會上傳。報告頁按「列印 / 存成 PDF」即可存檔。自動擷取僅供初篩，仍以 datasheet 原文為準。', cmpNeedTwo: '請先選兩份 PDF', cmpNoLib: '比對模組未載入（請重新整理頁面）', cmpBusy: '解析中…（大檔可能要幾秒）', cmpNoText: '這兩份 PDF 取不到文字，可能是掃描圖檔版；請改用原生文字版 datasheet', cmpPopup: '瀏覽器擋掉了新分頁，請允許此站開啟彈出視窗', cmpDone: '報告已在新分頁開啟', cmpFail: '比對失敗'
    },
    en: {
